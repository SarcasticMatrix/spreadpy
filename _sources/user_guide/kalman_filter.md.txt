...
We illustrate the three estimators on a classic pair: **GLD** (SPDR Gold Shares)
and **GDX** (VanEck Gold Miners ETF). Both are exposed to gold prices, but GDX
carries additional equity and operational risk, so the relationship drifts over time.
...
ols     = ConstantOLS()
rolling = RollingOLS(window=120)
kf      = KalmanFilter(alpha=1e-5)
kfv     = KalmanFilterWithVelocity(alpha=1e-6, alpha_dgam=1e-7)

# All estimators work on log-prices for homoscedastic residuals
y_log = PriceTimeSeries(np.log(y.series), name="log GLD")
x_log = PriceTimeSeries(np.log(x.series), name="log GDX")
...
ax.axhline(beta_ols.iloc[0], color="gray", linestyle="--", label="Constant OLS")
ax.plot(beta_rolling, color="steelblue", alpha=0.7, label="Rolling OLS (w=120)")
ax.plot(beta_kf,      color="tomato",    lw=1.5,   label="Kalman 2-state")
ax.plot(beta_kfv,     color="seagreen",  lw=1.5,   label="Kalman 3-state (velocity)")
ax.set_title("Hedge ratio $\\gamma_t$ — GLD / GDX", fontsize=13)
...
axes[0].plot(kf.normalized_spread_,  color="tomato",   label="2-state")
axes[0].plot(kfv.normalized_spread_, color="seagreen",  label="3-state")
axes[0].axhline(0, color="k", lw=0.5)
axes[0].set_title("Normalised spread $z_t$")
axes[0].legend()

axes[1].plot(kf.innovation_var_ts_,  color="tomato",   label="$S_t$ — 2-state")
axes[1].plot(kfv.innovation_var_ts_, color="seagreen",  label="$S_t$ — 3-state")
